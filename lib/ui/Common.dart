            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: <Widget>[
                  ...
                  SizedBox(height: 4),
                  FutureBuilder(
                    future: AudioRepository().fetchListening(a.id),
                    builder: (context, snapshot){
                      if (snapshot.hasData){
...
class FloatingActionPlay extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Consumer <AudioModel> (
      builder: (context, audio, child){
        if (audio.audioPlayer.state != null && (audio.audioPlayer.state == AudioPlayerState.PLAYING || audio.audioPlayer.state == AudioPlayerState.PAUSED) ) {
