...
class AudioModel extends ChangeNotifier {
  ...
  void _initializeAndPlay() async {
    ...
    await audioRepository.fetchListening(_currentAudio.id).then((Listening onValue) async {
    ...
    audioPlayer.onAudioPositionChanged.listen((Duration  p) async {
      ...
      if (p.inMilliseconds%20000 < 190 && p.inSeconds > 0) {
        await audioRepository.postListening(new Listening(audioId: _currentAudio.id, userId: UserModel().user.id, position: p.inSeconds, date: DateTime.now()));
      }
      _currentPosition = p;
      notifyListeners();
    });
    ...
    audioPlayer.onPlayerCompletion.listen((v) async{
      await audioRepository.postListening(new Listening(audioId: _currentAudio.id, userId: UserModel().user.id, position: _currentAudio.length, date: DateTime.now())).catchError((e){
    ...
    audioPlayer.onPlayerStateChanged.listen((v){
      print(v);
    });
